...
|---|---|---|---|---|
| `hydrabrain_cloud` | `openai` | cloud | `hydrabrain-cloud 1.0` | Yes |
...
HydraBrain also ships 76 disabled-by-default provider cards sourced from the
...
When a streaming provider returns token usage, HydraBrain records it directly.
If the provider omits usage, HydraBrain records a conservative character-based
...
- **Ollama**: `http://localhost:11434/v1` — `ollama serve`, or `ollama launch hydrabrain --model <model>`
...
<p align="center">
  <img src="../web/assets/hydrabrain-ollama-heart.png" alt="HydraBrain loves Ollama launch handoff" width="720">
</p>

HydraBrain supports Ollama today through the local OpenAI-compatible provider. A
new `ollama launch hydrabrain --model <model>` handoff can also configure HydraBrain
automatically, but it is not integrated into upstream Ollama yet. For now, try
it from the [`codex/ollama-hydrabrain-launch-handoff` branch of
`esokullu/ollama`](https://github.com/esokullu/ollama/tree/codex/ollama-hydrabrain-launch-handoff);
we hope Ollama will integrate it upstream.

```bash
git clone https://github.com/esokullu/ollama.git
cd ollama
git switch codex/ollama-hydrabrain-launch-handoff
cmake -S . -B build -G Ninja -DOLLAMA_MLX_BACKENDS=
cmake --build build --parallel 8

OLLAMA_ORIGINS="chrome-extension://*,moz-extension://*" ./ollama serve
./ollama launch hydrabrain --model <model>
```
...
Each non-HydraBrain provider config includes a persisted `configured` flag. An
...
**Selected** for chat. HydraBrain Cloud is always selectable without being marked
...
Deprecated provider entries (`hydrabrain`, `openai_subscription`,
`claude_subscription`) are filtered out.

### Cost Allowances

Settings exposes session and total cloud cost allowances. The agent prefers a provider-reported `usage.cost`/`usage.cost_usd` value when present (OpenRouter reports this directly). For direct cloud providers that only return token counts, HydraBrain estimates spend from the provider config fields:
