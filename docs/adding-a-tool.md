...
This guide walks through adding a new tool to the HydraBrain agent — from schema definition to execution dispatch to result handling.
...
- **Core tool**: product-owned browser, DOM, network, download, scheduler, or
  privileged behavior implemented in HydraBrain source. Use the full checklist
  below.
- **Skill tool**: user-importable, removable HTTP or download-job integration
  declared in a skill's `hydrabrain-tools` manifest. Use this when the tool is
  best treated as a trusted third-party extension rather than a HydraBrain core
  primitive.
...
Add a fenced `hydrabrain-tools` JSON block to the skill markdown:
...
`hydrabrain-skill` is optional and is stripped from the loaded prompt. Its
...
Without metadata, HydraBrain infers the first prose paragraph as the summary and
