...
HydraBrain accepts slash commands as the first thing on a line in the input box.
...
| `/workflow --export <id>` | Download a sanitized portable `hydrabrain-workflow/1` JSON file |
...
| `/dangerously-skip-permissions` | **Global permission-prompt bypass.** Turns off `Ask before consequential actions` without opening Settings. HydraBrain will act without per-site prompts until you re-enable the setting. |
...
HydraBrain discards it and continues from DOM and accessibility data.
...
`checkout-before.png` and `checkout-after.png`; without `--save-as`, HydraBrain
uses timestamped filenames.

For the Chrome diagnostic suffix, HydraBrain may reactivate the originating run
...
- **Settings snapshots** use `hydrabrain-config/1` and include all portable
...
- **Saved workflows** use a separate `hydrabrain-workflow/1` schema; they are not
  ...
  recorded origin and URL family. At run time HydraBrain resolves a fresh
...
- **Portable workflow files** contain the raw sanitized `hydrabrain-workflow/1`
  ...
  safely move between Chrome, Firefox, and HydraBrain Cloud.
