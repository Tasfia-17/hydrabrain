...
that HydraBrain loads into a run **only when it is relevant**. Manage them under
...
An optional fenced `hydrabrain-skill` JSON block can declare:
...
HydraBrain also recognizes the required `name` and `description` YAML frontmatter
...
`hydrabrain-skill` block still take precedence.

This is instruction-only compatibility. HydraBrain imports one text document; it
does not fetch bundled `scripts/`, `references/`, or `assets/`, execute skill
code, or treat the Agent Skills `allowed-tools` field as a HydraBrain permission
or tool manifest. Use `hydrabrain-tools` for HydraBrain HTTP tools. HydraBrain
recognizes `hydrabrain-skill` and `hydrabrain-tools` fences only in the Markdown
...
a fenced `hydrabrain-tools` JSON manifest.
...
Rewrites prose HydraBrain composes for you, such as an email reply or a post, so
