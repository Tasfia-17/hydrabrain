# HydraBrain Architecture

> Version 25.8.5

## Overview

HydraBrain is a browser extension that gives an LLM control over the user's active browser tab. The user types a natural-language instruction in a side panel, and an autonomous agent loop calls the LLM, executes tool calls (click, type, navigate, read page state, etc.), feeds results back to the LLM, and repeats until the task is done.
...
for an allowlisted, default-resolved `hydrabrain-config/1` snapshot, and
...
4. **Manage side panel visibility**: per-window "HydraBrain" tab group controls where the panel is enabled
...
Nvidia NIM, Groq, Together AI, Fireworks, z.ai, OpenRouter, HydraBrain Cloud,
...
- Routing catalog: optional fenced `hydrabrain-skill` JSON supplies a summary
  ...
  name and `hydrabrain-skill` metadata retain precedence.
...
- Prompt instructions: `buildCustomSkillsPrompt()` strips both metadata and
  `hydrabrain-tools` fences, strips valid Agent Skills frontmatter, then appends
...
reinforced by HydraBrain's untrusted-content wrappers and the loader description,
...
`exportedByHydraBrainVersion` while retaining the backward-compatible
`hydrabrain-trace/1` schema. Legacy runs are labeled with an unavailable recording
...
normalizes its replayable actions into `hydrabrain-workflow/1`, stored under
...
- **WebMCP** — `WebMCP.enable` maintains a bounded live catalog and
  `WebMCP.invokeTool` executes a page-registered structured capability. HydraBrain
...
| File upload | CDP path or `downloadId` | `downloadId` re-fetch or HydraBrain file picker; no arbitrary local path |
