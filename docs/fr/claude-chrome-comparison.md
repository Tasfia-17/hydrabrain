# HydraBrain vs Extension Claude Chrome

Cette note compare le checkout HydraBrain local avec `../hydrabrain-claude`, un
...
| Domaine | HydraBrain | Extension Claude Chrome |
|---|---|---|
...
### Outils de compétences dynamiques HydraBrain

HydraBrain dispose de deux classes d'outils déclarées dans les blocs Markdown de
compétences :
...
| Capacité | HydraBrain | Claude Chrome |
|---|---|---|
...
| Requête réseau | `fetch_url` / `research_url`, avec des règles de mutation d'API spécifiques à HydraBrain et `/allow-api` pour les méthodes mutantes. | Aucun outil de requête générique trouvé. Des logs réseau de débogage existent via `read_network_requests`. |
...
| Flux de travail GIF/vidéo | L'enregistrement via slash existe dans HydraBrain Chrome, mais pas en tant qu'outils appelables par le modèle. | `gif_creator` est appelable par le modèle et peut enregistrer/exporter des sessions d'automatisation navigateur en GIF. |

## Adaptateurs spécifiques aux sites

### HydraBrain

HydraBrain dispose d'un véritable système d'adaptateurs de sites :
...
- Si la navigation passe à un adaptateur correspondant différent en cours de
  conversation, HydraBrain injecte un nouveau message utilisateur
...
Les recherches de marqueurs d'adaptateurs de style HydraBrain n'ont trouvé que
...
- HydraBrain dispose d'une augmentation d'invite spécifique au site en tant que
...
Idées potentiellement utiles de Claude pour HydraBrain :
...
- Des primitives de liste et d'exécution de raccourcis/flux de travail, si
  HydraBrain souhaite une couche de flux de travail réutilisable distincte des
...
- Regrouper trop d'opérations navigateur déterministes dans un seul schéma
  `computer` si HydraBrain souhaite préserver sa sémantique d'outils étroite,
