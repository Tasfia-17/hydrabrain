...
accompagné de son propre manifeste d'outils — que HydraBrain charge dans une
...
Un bloc JSON `hydrabrain-skill` optionnel peut déclarer :
...
HydraBrain reconnaît également les métadonnées YAML obligatoires `name` et
...
un bloc `hydrabrain-skill` restent prioritaires.

Cette compatibilité concerne uniquement les instructions. HydraBrain importe un
...
d'outils HydraBrain. Utilisez `hydrabrain-tools` pour les outils HTTP HydraBrain.
HydraBrain ne reconnaît les blocs `hydrabrain-skill` et `hydrabrain-tools` que dans
...
tâche de téléchargement de courte durée, via un manifeste JSON `hydrabrain-tools`.
...
Réécrit la prose que HydraBrain rédige pour vous, par exemple une réponse à un
