...
|---|---|---|---|---|
| `hydrabrain_cloud` | `openai` | cloud | `hydrabrain-cloud 1.0` | Oui |
...
HydraBrain ajoute 76 cartes désactivées par défaut depuis l’instantané du
...
directement ; si elle est absente, HydraBrain enregistre une estimation prudente
...
- **Ollama** : `http://localhost:11434/v1` — `ollama serve`, ou `ollama launch hydrabrain --model <model>`
...
<p align="center">
  <img src="../../web/assets/hydrabrain-ollama-heart.png" alt="HydraBrain adore le relais de lancement Ollama" width="720">
</p>

HydraBrain prend aujourd'hui en charge Ollama via le fournisseur local compatible
OpenAI. Un nouveau relais `ollama launch hydrabrain --model <model>` peut aussi
configurer HydraBrain automatiquement, mais il n'est pas encore intégré à Ollama
en amont. Pour l'instant, essayez-le depuis la [branche
`codex/ollama-hydrabrain-launch-handoff` de
...
Les entrées de fournisseur obsolètes (`hydrabrain`, `openai_subscription`,
`claude_subscription`) sont filtrées.

### Plafonds de Coût

Les paramètres exposent des plafonds de coût cloud pour la session et le total. L'agent préfère une valeur `usage.cost`/`usage.cost_usd` rapportée par le fournisseur lorsqu'elle est présente (OpenRouter la rapporte directement). Pour les fournisseurs cloud directs qui ne retournent que des compteurs de tokens, HydraBrain estime les dépenses à partir des champs de configuration du fournisseur :
