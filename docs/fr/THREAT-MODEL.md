# HydraBrain — Modèle de menace et mesures d'atténuation pour la sécurité de l'agent
...
3. Où l'inférence s'exécute-t-elle, et qu'est-ce qui quitte l'appareil ? (Le fournisseur sélectionné le détermine : HydraBrain Cloud et les fournisseurs cloud configurés par l'utilisateur reçoivent le contexte de la requête, tandis que les fournisseurs locaux conservent les requêtes d'inférence sur la machine.)
