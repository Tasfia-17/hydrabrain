...
- **HydraBrain Cloud** : les requêtes passent par `api.hydrabrain.one` ; Aider à
  améliorer HydraBrain est activé par défaut et, tant qu'il reste activé,
...
- **Fournisseurs cloud configurés par l'utilisateur** : OpenAI, Anthropic,
  ...
  et ne sont jamais collectées par HydraBrain
...
sont jamais collectées par HydraBrain. Les requêtes HydraBrain Cloud sont traitées
et peuvent être conservées conformément à la section détaillée de la
[documentation anglaise](../privacy-and-data-flow.md#hydrabrain-cloud-improvement-data).
...
synchronisées séparément avec HydraBrain.
...
l'un de ces outils, HydraBrain envoie uniquement l'URL actuelle ou fournie par le
