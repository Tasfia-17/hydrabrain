...
- **HydraBrain Cloud**: requests go through `api.hydrabrain.one`; selected interactions may be retained and used for evaluation, improvement, fine-tuning, and training while Help Improve HydraBrain is enabled
- **Bring-your-own cloud providers**: OpenAI, Anthropic, Google Gemini, Mistral, DeepSeek, xAI, Groq, OpenRouter, etc. — requests go directly to the provider using the user's credentials and are never collected by HydraBrain
- **Local providers**: llama.cpp, Ollama, LM Studio, Jan, vLLM, SGLang, LocalAI — data stays on the user's machine

Local-model and bring-your-own API requests are never collected by HydraBrain. HydraBrain Cloud requests are processed and may be retained as described below.

### HydraBrain Cloud improvement data

Help Improve HydraBrain is available under Settings -> General and is
on by default. When it is on, HydraBrain may retain eligible Cloud prompts, model
...
and image URLs are excluded from HydraBrain's improvement database. The extension
sends the current preference, stable conversation id, and an allowlisted
generation label with every HydraBrain Cloud model request. It never attaches
those collection fields to local or bring-your-own providers.

Current clients explicitly send `X-HydraBrain-Help-Improve: 1` or `0`. Older
HydraBrain Cloud clients that send neither the preference header nor a session id
...
user's own credentials never pass through HydraBrain Cloud and are never eligible
for HydraBrain training.

For eligible completed generations, MySQL is HydraBrain's canonical store. The
...
Before retained Cloud interactions are used for model development, HydraBrain
...
the stored copies are not separately synced to HydraBrain.

### Trace Recorder

When enabled (Settings → Display → "Record traces"), every agent run is written to an IndexedDB database (`hydrabrain_traces`):
...
separate `hydrabrain-workflow/1` record in browser local storage
...
workflow preferences. HydraBrain rejects obvious secrets and credential-like text,
...
`/export --config` creates a local plaintext `hydrabrain-config/1` JSON file, and
...
state, the HydraBrain Cloud device ID, conversations, traces, scheduled jobs,
usage counters, or accumulated spend. Import does not upload the JSON to
HydraBrain Cloud or to the configured LLM provider.

### Optional Encrypted Cloud Sync

Active HydraBrain Cloud subscribers may explicitly enable encrypted profile sync in
...
HydraBrain Cloud receives only ciphertext and cryptographic/version metadata. It
...
Settings → General → Advanced before HydraBrain sends its tool schemas or prompt
guidance to the configured LLM. On supporting Chrome pages, HydraBrain can then
...
registration frame. HydraBrain keeps a bounded in-memory per-tab catalog, assigns
...
separate product-telemetry endpoint. When HydraBrain Cloud is selected, the model
request itself goes to `api.hydrabrain.one` and is subject to the Cloud data-use
...
1. **HydraBrain Cloud model calls** to `https://api.hydrabrain.one/v1` (when HydraBrain Cloud is selected; the Help Improve HydraBrain preference is sent with each request)
...
7. **Encrypted Cloud Sync calls** to `https://api.hydrabrain.one/v1/sync` (only after a subscriber explicitly enables sync; vault content is encrypted before upload)
...
Trace records store the HydraBrain version that created each run. Conversation
...
`exportedByHydraBrainVersion` plus the run's `hydrabrainVersion` when available.
...
HydraBrain sends only the current or model-provided URL, plus declared options
...
is prompt-only, and declares no external tool or endpoint. It guides HydraBrain's
...
skill itself creates no additional network request. When the user asks HydraBrain
...
result and model response are also retained locally in the `hydrabrain_traces`
