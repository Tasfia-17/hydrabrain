# HydraBrain vs Claude Chrome Extension

This note compares the local HydraBrain checkout with `../hydrabrain-claude`, a
...
- `docs/hydrabrain-tool-tiers.xlsx`
...
- `../hydrabrain-claude/manifest.json`
- `../hydrabrain-claude/settings.html`
- `../hydrabrain-claude/settings.js`
- `../hydrabrain-claude/assets/service-worker.js`
- `../hydrabrain-claude/assets/mcpPermissions.js`
- `../hydrabrain-claude/assets/PermissionManager.js`
- `../hydrabrain-claude/assets/sidepanel.js`
- `../hydrabrain-claude/assets/accessibility-tree.js`
...
| Area | HydraBrain | Claude Chrome extension |
|---|---|---|
...
| Dynamic extension model | User/imported skills can inject prompt text and declare `hydrabrain-tools` runtime tools. | Native/MCP and shortcuts are the extension points visible in the deobfuscated tree; no equivalent user-editable Markdown tool manifest was found. |

## HydraBrain Tool Surface
...
### HydraBrain Dynamic Skill Tools

HydraBrain has two tool classes declared inside skill Markdown fences:
...
| Capability | HydraBrain | Claude Chrome |
|---|---|---|
...
| Network fetch | `fetch_url` / `research_url`, with HydraBrain-specific API mutation rules and `/allow-api` for mutating methods. | No generic fetch tool recovered. Debug network logs exist through `read_network_requests`. |
...
| GIF/video workflow | Slash-driven recording exists in HydraBrain Chrome, but not as model-callable tools. | `gif_creator` is model-callable and can record/export browser automation sessions as GIF. |

## Website-Specific Adapters

### HydraBrain

HydraBrain has a real site-adapter system:
...
- If navigation moves to a different matching adapter mid-conversation, HydraBrain
...
inspected Claude tree. Searches for HydraBrain-style adapter markers only found
...
- HydraBrain has website-specific prompt augmentation as a first-class browser
  agent feature.
...
Potentially useful Claude ideas for HydraBrain:
...
- Shortcut/workflow list and execution primitives, if HydraBrain wants a reusable
...
- Collapsing too many deterministic browser operations into one `computer`
  schema if HydraBrain wants to preserve its current narrow, auditable tool
  semantics.
