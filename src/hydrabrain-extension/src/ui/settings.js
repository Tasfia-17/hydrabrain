...
helpImproveToggle?.addEventListener('change', async () => {
  await chrome.storage.local.set({ helpImproveHydraBrain: helpImproveToggle.checked }).catch(() => {});
});
...
btnProfileSyncAuth?.addEventListener('click', async () => {
  const email = (profileSyncEmail?.value || '').trim(); if (!email) return showProfileSyncResult(false, 'Enter your HydraBrain Cloud billing email.');
  try { profileSyncChallenge = await profileSyncAction('profile_sync_auth_start', { email }); showProfileSyncResult(true, 'Check your email, approve the HydraBrain Cloud sign-in link, then return here.');
