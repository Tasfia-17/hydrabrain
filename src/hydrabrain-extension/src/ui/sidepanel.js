/**
 * HydraBrain Side Panel — Chat UI logic.
...
(async function initOnboarding() {
  ...
  async function scanLocalModels() {
    ...
    try {
      ...
      if (active === 'hydrabrain_cloud' && providers.hydrabrain_cloud?.enabled !== false) {
        showCloudReady();
        return;
      }
      ...
      if (choices.length > 0) {
        ...
        console.warn('[HydraBrain] onboarding local-model scan failed:', errors);
...
const HYDRABRAIN_PROMOTION_ACTION_IDS = new Set(['tweet-hydrabrain', 'post-hydrabrain-linkedin']);
...
const SLASH_COMMANDS = [
  ...
  // HydraBrain commands
  {
    value: '/hydradb',
    usage: '/hydradb [query]',
    description: 'Query HydraDB knowledge graph across GitHub, Linear, Slack, Notion',
    action: 'query',
    acceptsPayload: true,
    outOfBand: true,
    options: [
      { value: '--metrics', description: 'Show fast/thinking usage and latency dashboard', action: 'metrics', outOfBand: true, disallowPayload: true },
      { value: '--demo', description: 'Run the 5 competition demo questions', action: 'demo', outOfBand: true, disallowPayload: true },
      { value: '--ingest-demo', description: 'Ingest demo cross-source dataset into HydraDB', action: 'ingest-demo', outOfBand: true, disallowPayload: true },
    ],
  },
  {
    value: '/sync',
    usage: '/sync [github|linear|slack|notion|all]',
    description: 'Trigger HydraDB connector sync to pull latest data',
    action: 'sync',
    acceptsPayload: true,
    outOfBand: true,
  },
...
function buildRunScreenshotFilenames(saveAs, date = new Date()) {
  ...
  const stem = (requested.replace(/\.png$/i, '') || `hydrabrain-run-${runCaptureTimestamp(date)}`).slice(0, 170);
  return {
...
function buildRunRecordingFilename(saveAs) {
  ...
  const stem = requested.replace(/\.webm$/i, '').replace(/[. ]+$/g, '') || 'hydrabrain-recording';
...
let selectedProviderId = 'hydrabrain_cloud';
let recommendedActionsCollapsed = false;
let hydrabrainPromotionHasAnimated = false;
...
async function persistChatHistorySnapshot(tabId, { refreshTabInfo = false } = {}) {
  ...
  await saveChatHistoryRecord({
    ...
    console.warn('[HydraBrain] failed to save chat history:', error);
  });
}
...
async function repairRestoredChatHistorySnapshot(tabId) {
  ...
  await repairChatHistoryRecordMessages(recordId, messages).catch((error) => {
    console.warn('[HydraBrain] failed to repair restored chat history:', error);
  });
}
...
async function resetChatHistoryStateForTab(tabId) {
  ...
  await Promise.all(Array.from(recordIdsToDelete).map((recordId) => (
    deleteChatHistoryRecord(recordId).catch((error) => {
      console.warn('[HydraBrain] failed to delete chat history:', error);
    })
  )));
...
async function refreshScheduledJobs({ tabId = null } = {}) {
  ...
  try {
    ...
    console.warn('[HydraBrain] failed to refresh scheduled jobs:', e);
    return [];
  }
}
...
// ─── HydraBrain: HydraDB side panel functions ──────────────────────────────

/**
 * Perform a HydraDB query and display results in the side panel.
 * Called by /hydradb [query] slash command.
 */
async function handleHydraDBQuery(query, tabId = currentTabId) {
  const loadingEl = addPersistentSlashMessage(systemHtml(
    `<div class="hydrabrain-panel">` +
    `<div class="hydrabrain-header">🧠 HydraBrain — Querying knowledge graph…</div>` +
    `<div class="hydrabrain-loading">🔍 <em>${escapeHtml(query)}</em></div>` +
    `</div>`,
  ));

  try {
    const res = await sendToBackground('hydrabrain_query', { query, tabId });
    if (currentTabId !== tabId) return;
    if (!res?.success) {
      loadingEl?.remove();
      addPersistentSlashMessage(systemHtml(
        `<div class="hydrabrain-panel hydrabrain-error">` +
        `<strong>HydraBrain error:</strong> ${escapeHtml(res?.error || 'Query failed')}` +
        `</div>`,
      ));
      return;
    }
    ...
    const chunksHtml = chunks.length === 0
      ? '<p><em>No results found. Try a different query or use --thinking mode.</em></p>'
      : chunks.map((c, i) => {
        const source = c.collection || 'unknown';
        const score = (c.score * 100).toFixed(0);
        const content = escapeHtml((c.content || '').slice(0, 400)) + (c.content?.length > 400 ? '…' : '');
        return `<div class="hydrabrain-chunk">` +
          `<div class="hydrabrain-chunk-header">` +
          `<span class="hydrabrain-source hydrabrain-source-${escapeHtml(source)}">${escapeHtml(source)}</span>` +
          `<span class="hydrabrain-score">${score}% match</span>` +
          `</div>` +
          `<div class="hydrabrain-chunk-body">${content}</div>` +
          `</div>`;
      }).join('');

    const modeIcon  = res.mode === 'thinking' ? '🧠' : '⚡';
    const modeLabel = res.mode === 'thinking' ? 'Thinking' : 'Fast';

    loadingEl?.remove();
    const msgEl = addPersistentSlashMessage(systemHtml(
      `<div class="hydrabrain-panel">` +
      `<div class="hydrabrain-header">🧠 HydraBrain — ${escapeHtml(query)}</div>` +
      `<div class="hydrabrain-meta">` +
      `${modeIcon} ${modeLabel} mode &nbsp;·&nbsp; ` +
      `⏱ ${res.latencyMs}ms &nbsp;·&nbsp; ` +
      `💰 $${res.costUsd} &nbsp;·&nbsp; ` +
      `📄 ${res.chunkCount} chunks` +
      `</div>` +
      chunksHtml +
      `</div>`,
    ));
    ...
    if (currentTabId === tabId) {
      addPersistentSlashMessage(systemHtml(
        `<div class="hydrabrain-panel hydrabrain-error">HydraBrain error: ${escapeHtml(err.message)}</div>`,
      ));
    }
  }
}
...
/**
 * Show HydraDB performance metrics dashboard.
 * Called by /hydradb --metrics
 */
async function handleHydraDBMetrics(tabId = currentTabId) {
  try {
    const res = await sendToBackground('hydrabrain_metrics', { tabId });
    if (currentTabId !== tabId) return;
    const m = res?.metrics || {};
    const fastBar  = '█'.repeat(Math.round((m.fastPct || 0) / 10));
    const slowBar  = '░'.repeat(10 - Math.round((m.fastPct || 0) / 10));

    addPersistentSlashMessage(systemHtml(
      `<div class="hydrabrain-panel">` +
      `<div class="hydrabrain-header">📊 HydraBrain Performance Dashboard</div>` +
      `<pre class="scratchpad-dump">` +
      `Session Queries:  ${m.totalQueries || 0}\n` +
      `────────────────────────────────\n` +
      `Fast Mode:      ${m.fastQueries || 0} (${m.fastPct || 0}%)  ${fastBar}${slowBar}\n` +
      `Thinking Mode:  ${m.thinkingQueries || 0} (${100 - (m.fastPct || 0)}%)\n\n` +
      `Avg Latency:    ${m.avgLatencyMs || 0}ms\n` +
      `Total Cost:     $${m.totalCostUsd || '0.0000'}\n` +
      `HydraDB Calls:  ${m.hydradbCalls || 0}\n` +
      `Session Time:   ${m.sessionMinutes || 0} min` +
      `</pre>` +
      `</div>`,
    ));
  } catch (err) {
    addPersistentSlashMessage(systemHtml(
      `<div class="hydrabrain-panel hydrabrain-error">Metrics error: ${escapeHtml(err.message)}</div>`,
    ));
  }
}

/**
 * Run all 5 demo competition questions with reasoning traces.
 * Called by /hydradb --demo
 */
async function handleHydraDBDemo(tabId = currentTabId) {
  const loadingEl = addPersistentSlashMessage(systemHtml(
    `<div class="hydrabrain-panel">` +
    `<div class="hydrabrain-header">🧠 HydraBrain — Running Demo Questions…</div>` +
    `<div class="hydrabrain-loading">Running 5 multi-hop reasoning questions against HydraDB…</div>` +
    `</div>`,
  ));
  try {
    const res = await sendToBackground('hydrabrain_demo', { tabId });
    if (currentTabId !== tabId) return;
    loadingEl?.remove();
    ...
    const html = results.map((r, idx) => {
      const hopsHtml = (r.hops || []).map(h => {
        const icon = h.mode === 'thinking' ? '🧠' : '⚡';
        const status = h.error ? '❌' : (h.chunks?.length > 0 ? '✅' : '⚠️');
        return `  ${status} Hop ${h.hopNumber}: ${escapeHtml(h.label)} ` +
          `[${icon} ${h.mode} · ${h.latencyMs}ms · $${h.costUsd.toFixed(3)}]`;
      }).join('\n');

      return `<div class="hydrabrain-chunk">` +
        `<div class="hydrabrain-chunk-header"><strong>Q${idx + 1}: ${escapeHtml(r.question)}</strong></div>` +
        `<pre class="scratchpad-dump">${hopsHtml}\n\nTotal: ${r.totalLatencyMs}ms | $${r.totalCostUsd} | ` +
        `${r.fastHops} fast, ${r.thinkingHops} thinking</pre>` +
        `</div>`;
    }).join('');

    const totalLatency = results.reduce((s, r) => s + (r.totalLatencyMs || 0), 0);
    const totalCost    = results.reduce((s, r) => s + (r.totalCostUsd || 0), 0).toFixed(4);

    const msgEl = addPersistentSlashMessage(systemHtml(
      `<div class="hydrabrain-panel">` +
      `<div class="hydrabrain-header">🧠 HydraBrain Demo — 5 Competition Questions</div>` +
      `<div class="hydrabrain-meta">Total: ${totalLatency}ms | $${totalCost}</div>` +
      html +
      `</div>`,
    ));
    addScratchpadCopyButton(msgEl);
    ...
    if (currentTabId === tabId) {
      addPersistentSlashMessage(systemHtml(
        `<div class="hydrabrain-panel hydrabrain-error">Demo error: ${escapeHtml(err.message)}</div>`,
      ));
    }
  }
}
 ...
 * Called by /hydradb --ingest-demo
 */
async function handleHydraDBIngestDemo(tabId = currentTabId) {
  const loadingEl = addPersistentSlashMessage(systemHtml(
    `<div class="hydrabrain-panel">` +
    `<div class="hydrabrain-header">🧠 HydraBrain — Ingesting Demo Data…</div>` +
    `<div class="hydrabrain-loading">Ingesting cross-source scenario: GitHub, Linear, Slack, Notion…</div>` +
    `</div>`,
  ));
  try {
    const res = await sendToBackground('hydrabrain_ingest_demo', { tabId });
    if (currentTabId !== tabId) return;
    loadingEl?.remove();
    const r = res?.report || {};
    addPersistentSlashMessage(systemHtml(
      `<div class="hydrabrain-panel">` +
      `<div class="hydrabrain-header">✅ HydraBrain — Demo Data Ingested</div>` +
      `<pre class="scratchpad-dump">` +
      `Ingested: ${r.successCount || 0} documents\n` +
      `Failed:   ${r.failCount || 0}\n` +
      `Latency:  ${r.totalLatencyMs || 0}ms total\n\n` +
      `Sources:\n` +
      (r.ingested || []).map(d => `  ✓ [${d.source}] ${d.title}`).join('\n') +
      (r.failed?.length ? '\n\nFailed:\n' + r.failed.map(d => `  ✗ [${d.source}] ${d.title}: ${d.error}`).join('\n') : '') +
      `</pre></div>`,
    ));
  } catch (err) {
    loadingEl?.remove();
    if (currentTabId === tabId) {
      addPersistentSlashMessage(systemHtml(
        `<div class="hydrabrain-panel hydrabrain-error">Ingest error: ${escapeHtml(err.message)}</div>`,
      ));
    }
  }
}
...
/**
 * Trigger HydraDB connector sync.
 * Called by /sync [connector]
 */
async function handleHydraDBSync(connector, tabId = currentTabId) {
  const label = connector === 'all' ? 'all connectors' : connector;
  const loadingEl = addPersistentSlashMessage(systemHtml(
    `<div class="hydrabrain-panel">` +
    `<div class="hydrabrain-header">🔄 HydraBrain — Syncing ${escapeHtml(label)}…</div>` +
    `</div>`,
  ));
  try {
    const res = await sendToBackground('hydrabrain_sync', { connector, tabId });
    if (currentTabId !== tabId) return;
    loadingEl?.remove();
    ...
    addPersistentSlashMessage(systemHtml(
      `<div class="hydrabrain-panel">` +
      `<div class="hydrabrain-header">✅ HydraBrain — Sync Triggered</div>` +
      `<pre class="scratchpad-dump">` +
      (synced.length ? `Synced: ${synced.join(', ')}\n` : '') +
      (failed.length ? `Failed: ${failed.join(', ')}\n` : '') +
      `\nData will be available in ~60 seconds.` +
      `</pre></div>`,
    ));
  } catch (err) {
    loadingEl?.remove();
    if (currentTabId === tabId) {
      addPersistentSlashMessage(systemHtml(
        `<div class="hydrabrain-panel hydrabrain-error">Sync error: ${escapeHtml(err.message)}</div>`,
      ));
    }
  }
}
...
function savedWorkflowDownloadFilename(name) {
  ...
  return `${stem}.hydrabrain-workflow.json`;
}
...
function requestSavedWorkflowFile(tabId) {
  ...
  input.type = 'file';
  input.accept = '.json,.hydrabrain-workflow.json,application/json';
...
async function init() {
  ...
  await loadProviders();
  await testConnection({ skipHydraBrainCloud: true });
    ...
    if (e.shiftKey) {
      try {
        ...
        if (response?.log?.length) {
          console.group('%c[HydraBrain Deep Verbose] %d entries', 'color:#7c3aed;font-weight:bold', response.log.length);
          ...
          console.log('%c[HydraBrain Deep Verbose] No entries yet — run a query first.', 'color:#7c3aed');
        }
      } catch (err) {
        console.error('[HydraBrain Deep Verbose] Failed to fetch debug log:', err);
      }
...
function createHydraBrainPromotionIcon(actionId) {
  ...
  path.setAttribute('d', actionId === 'post-hydrabrain-linkedin'
...
function animateHydraBrainPromotionOnce() {
  if (hydrabrainPromotionHasAnimated || recommendedActionsCollapsed) return;
  ...
  hydrabrainPromotionHasAnimated = true;
}
...
function setRecommendedActionsCollapsed(collapsed, { persist = true } = {}) {
  ...
  animateHydraBrainPromotionOnce();
  if (persist) {
...
async function refreshRecommendedActions() {
  ...
  try {
    ...
    const hydrabrainPromotionVariant = Math.random() < 0.5 ? 'linkedin' : 'x';
    const actions = buildRecommendedActions(pageInfo, { max: 4, hydrabrainPromotionVariant });
    recommendedActionsListEl.replaceChildren();
    actions.forEach((action) => {
      ...
      if (HYDRABRAIN_PROMOTION_ACTION_IDS.has(action.id)) {
        btn.classList.add('recommended-action-chip-promotion');
        btn.prepend(createHydraBrainPromotionIcon(action.id));
      }
...
function appendProviderPickerOption(id, name, meta) {
  ...
  // HydraBrain mark (and other brand chips) don't compete with the chrome.
...
async function loadProviders() {
  try {
    ...
    const cloudConfig = res.providers.hydrabrain_cloud || { label: 'HydraBrain Cloud' };
    const cloudLabel = cloudConfig.label || 'HydraBrain Cloud';
    const cloudGroup = document.createElement('optgroup');
    cloudGroup.label = t('sp.providers.no_setup_group');
    const cloudOption = document.createElement('option');
    cloudOption.value = 'hydrabrain_cloud';
    ...
    providerPickerLabelById.set('hydrabrain_cloud', cloudLabel);
    appendProviderPickerGroup(cloudGroup.label);
    appendProviderPickerOption('hydrabrain_cloud', cloudLabel, t('sp.providers.no_setup'));

    const configuredEntries = Object.entries(res.providers)
      .filter(([id, config]) => id !== 'hydrabrain_cloud' && config?.configured === true);
    ...
    const selectableProviderIds = new Set(['hydrabrain_cloud', ...configuredEntries.map(([id]) => id)]);
    selectedProviderId = selectableProviderIds.has(res.active) ? res.active : 'hydrabrain_cloud';
...
function isHydraBrainCloudProviderSelected() {
  return providerSelect?.value === 'hydrabrain_cloud';
}
...
async function testConnection(options = {}) {
  ...
  if (options.skipHydraBrainCloud && providerId === 'hydrabrain_cloud') {
...
function screenshotDownloadFilename(pageUrl = '', fullPage = false) {
  const prefix = screenshotFilenamePrefix(pageUrl) || 'hydrabrain';
...
async function parseSlashCommands(text, tabId = currentTabId, options = {}) {
  ...
  // ─── HydraBrain slash commands ─────────────────────────────────────────────
  if (command.value === '/hydradb') {
    if (action === 'query' || action === 'show') {
      await handleHydraDBQuery(payload || 'Show recent activity across all sources', tabId);
      return '';
    }
    if (action === 'metrics') {
      await handleHydraDBMetrics(tabId);
      return '';
    }
    if (action === 'demo') {
      await handleHydraDBDemo(tabId);
      return '';
    }
    if (action === 'ingest-demo') {
      await handleHydraDBIngestDemo(tabId);
      return '';
    }
    // Default: show query panel
    await handleHydraDBQuery(payload || 'Show recent activity across all sources', tabId);
    return '';
  }

  if (command.value === '/sync') {
    const connector = payload?.trim() || 'all';
    await handleHydraDBSync(connector, tabId);
    return '';
  }
  // ──────────────────────────────────────────────────────────────────────────
  ...
  if (command.value === '/export' && action === 'config') {
    try {
      ...
      a.href = url;
      a.download = `hydrabrain-config-${Date.now()}.json`;
  ...
  if (command.value === '/export' && action === 'traces') {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `hydrabrain-traces-${Date.now()}.md`;
    document.body.appendChild(a);
    try {
      a.click();
    } finally {
  ...
  if (command.value === '/export' && action === 'conversation') {
    ...
    const hydrabrainVersion = chrome.runtime.getManifest().version || 'unknown';
    let md = `# HydraBrain Conversation\n\n_Exported with HydraBrain v${hydrabrainVersion}_\n\n`;
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `hydrabrain-chat-${Date.now()}.md`;
    document.body.appendChild(a);
    try {
      a.click();
    } finally {
...
function handleAgentUpdateMessage(msg) {
  ...
  if (msg.type === 'scheduled_job') {
    handleScheduledJobEvent(msg.data, msg.tabId).catch((err) => {
      console.warn('[HydraBrain] failed to handle scheduled job event:', err);
    });
    return;
  }
...
function renderClarifyCard(data) {
  ...
  if (data.submitConfirmation) {
    ...
    qEl.textContent = String(data.question || `HydraBrain wants to submit this form on ${host}.`).slice(0, 600);
...
function submitPlanReview(card, tabId, planId, action, editedText) {
  ...
  const failureText = (error) => isBackgroundConnectionError(error)
    ? 'HydraBrain reloaded or the background worker stopped before this plan could be approved. Reload the sidebar and try again.'
    : expiredText();
...
// HydraBrain Cloud returns a 402 with a trailing "Subscribe for more usage: <url>"
...
function scheduleMathRender() {
  ...
  _mathRenderTimer = setTimeout(() => {
    ...
    try {
      ...
      console.warn('[hydrabrain] math render failed:', e);
    }
  }, 50);
}
...
function formatBackgroundSendError(action, message) {
  if (isBackgroundConnectionError(message)) {
    return `HydraBrain extension connection was lost while sending "${action}". Reload the sidebar/extension and try again.`;
  }
  return message;
}
    ...
    chrome.runtime.sendMessage(
      ...
      (response) => {
        if (chrome.runtime.lastError) {
          ...
          reject(new Error(`No response from HydraBrain background for "${action}". The background script may have restarted or crashed; reload the sidebar/extension and check the extension console for the original error.`));
