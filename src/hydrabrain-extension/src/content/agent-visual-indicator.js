/**
 * HydraBrain Agent Visual Indicator (content script)
 *
 * Renders an animated purple inset glow around the page viewport while
 * the agent is operating on this tab, a "Stop HydraBrain" floating button,
 * and a large HydraBrain-purple cursor/outline on the element currently
 * being acted on. The base indicator follows Anthropic's Claude-for-Chrome
 * extension pattern, recolored for HydraBrain's accent (#6c63ff).
 *
...
(function () {
  if (window.__hydrabrainAgentIndicatorInjected) return;
  window.__hydrabrainAgentIndicatorInjected = true;
  ...
  function injectStyles() {
    if (document.getElementById('hydrabrain-agent-styles')) return;
    const style = document.createElement('style');
    style.id = 'hydrabrain-agent-styles';
    style.textContent = `
      @keyframes hydrabrain-pulse {
        0% {
          box-shadow:
      ...
      @keyframes hydrabrain-target-pop {
        0% { opacity: 0; }
        100% { opacity: 1; }
      }

      @keyframes hydrabrain-target-ring {
        0%, 100% {
          box-shadow:
  ...
  function createBorder() {
    ...
    el.id = 'hydrabrain-agent-glow-border';
    el.style.cssText = `
      position: fixed;
      ...
      animation: hydrabrain-pulse 2s ease-in-out infinite;
      box-shadow:
        inset 0 0 10px rgba(108, 99, 255, 0.5),
        inset 0 0 20px rgba(108, 99, 255, 0.3),
        inset 0 0 30px rgba(108, 99, 255, 0.1);
  ...
  function createStopButton() {
    ...
    container.id = 'hydrabrain-agent-stop-container';
    ...
    button.id = 'hydrabrain-agent-stop-button';
    button.type = 'button';
    button.innerHTML = `
      ...
      <span style="vertical-align: middle;">Stop HydraBrain</span>
    `;
  ...
  function createTargetOutline() {
    const el = document.createElement('div');
    el.id = 'hydrabrain-agent-target-outline';
    el.style.cssText = `
      position: fixed;
      pointer-events: none;
      z-index: 2147483645;
      ...
      transition:
        ...
        width 0.18s ease,
        height 0.18s ease;
      animation: hydrabrain-target-ring 1.3s ease-in-out infinite;
    `;
    return el;
  }
  ...
  function createTargetCursor() {
    const el = document.createElement('div');
    el.id = 'hydrabrain-agent-target-cursor';
    ...
    el.style.cssText = `
      ...
      animation: hydrabrain-target-pop 0.18s ease-out;
  ...
  function show() {
    ...
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        ...
        const btn = stopContainerEl?.querySelector('#hydrabrain-agent-stop-button');
        if (btn) {
  ...
  window.__hydrabrainAgentIndicator = { showTarget, showTargetRect, hideTarget: hideTargetCursor };
