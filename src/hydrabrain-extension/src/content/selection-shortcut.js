/**
 * Floating HydraBrain shortcut for selected page text.
 *
 ...
 */
(function () {
  if (window.top !== window || window.__hydrabrainSelectionShortcutInjected) return;
  window.__hydrabrainSelectionShortcutInjected = true;
  ...
  function ensureSurface() {
    ...
    host.id = 'hydrabrain-selection-shortcut-host';
    ...
    shadow.innerHTML = `
      ...
      <button class="shortcut" type="button" aria-label="Ask HydraBrain about selected text" title="Ask HydraBrain" hidden>
        <span class="shortcut-icon" aria-hidden="true">?</span>
      </button>
      <div class="popup" role="dialog" aria-label="Ask HydraBrain about selected text" hidden>
        <div class="main-view">
          <div class="actions">
          ...
          <div class="question-wrap">
            <textarea maxlength="2000" rows="3" aria-label="Ask HydraBrain a question" placeholder="Ask HydraBrain…"></textarea>
  ...
  async function submitSelection(action, customQuestion = '', language = '') {
    ...
    try {
      ...
      if (response.requiresManualOpen) showToast('Sent to HydraBrain. Open the sidebar if it doesn’t start.');
    } catch {
      showToast('Couldn’t send to HydraBrain. Use the right-click menu and choose “Ask HydraBrain about this”.');
    } finally {
      submitting = false;
    }
  }
  ...
  window.__hydrabrainSelectionShortcut = {
