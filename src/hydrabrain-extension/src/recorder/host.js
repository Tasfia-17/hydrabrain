...
const RECORDING_SAFETY_ALARM_NAME = 'hydrabrain-recording-safety-cap';
...
function normalizeRecordingFilename(value) {
  ...
  const stem = filename.replace(/\.webm$/i, '').replace(/[. ]+$/g, '') || 'hydrabrain-recording';
...
function scheduleRecordingSafetyWatchdog(state = recordingState) {
  ...
  recordingSafetyTimeout = setTimeout(() => {
    stopRecordingForSafetyCap().catch((e) => {
      console.warn('[HydraBrain] recording safety cap failed:', e);
    });
  }, delay);
...
try {
  chrome.alarms?.onAlarm?.addListener?.((alarm) => {
    ...
    stopRecordingForSafetyCap().catch((e) => {
      console.warn('[HydraBrain] recording safety alarm failed:', e);
    });
  });
} catch {}
...
async function beforeRecordingFinalize(beforeFinalizeRecording) {
  ...
  try {
    ...
    console.warn('[HydraBrain] recording finalize provider preload failed:', e);
  }
}
...
export async function stopTabRecording(opts = {}) {
  ...
  const stamp = new Date()
    ...
    .replace(/T/, '_')
    .slice(0, 19);
  const filename = recordingState.filename || `hydrabrain-recording-${stamp}.webm`;
  ...
  if (wantTranscribe) {
    runTranscription({
      ...
      console.error('[HydraBrain] runTranscription crashed:', e);
    });
  }
