...
/**
 * HydraBrain Service Worker (Background Script)
...
const CONTEXT_MENU_ASK_SELECTION_ID = 'hydrabrain-ask-selection';
const CONTEXT_MENU_OPEN_CHAT_ID = 'hydrabrain-selection-open-chat';
const CONTEXT_MENU_ACTION_PREFIX = 'hydrabrain-selection-action-';
const CONTEXT_MENU_TRANSLATE_ID = 'hydrabrain-selection-translate';
const CONTEXT_MENU_TRANSLATE_PREFIX = 'hydrabrain-selection-translate-';
const CONTEXT_MENU_GENERIC_ASK_ID = 'hydrabrain-selection-generic-ask';
...
function createContextMenus() {
  ...
  const create = (item) => {
    chrome.contextMenus.create(item, () => {
      ...
      if (err && !/duplicate/i.test(String(err.message || err))) {
        console.warn('[HydraBrain] Failed to create context menu:', err.message || err);
      }
    });
  };
  ...
  chrome.contextMenus.removeAll(() => {
    ...
    create({
      ...
      title: 'Ask HydraBrain about this',
      contexts: ['selection'],
    });
    ...
    create({ id: 'hydrabrain-selection-separator-1', parentId: CONTEXT_MENU_ASK_SELECTION_ID, type: 'separator', contexts: ['selection'] });
    ...
    create({ id: 'hydrabrain-selection-separator-2', parentId: CONTEXT_MENU_ASK_SELECTION_ID, type: 'separator', contexts: ['selection'] });
...
function scheduleUserMemoryExtractionDrain(delayMs = USER_MEMORY_EXTRACTION_DELAY_MS) {
  ...
  userMemoryExtractionTimer = setTimeout(() => {
    ...
    drainUserMemoryExtractionQueue().catch((error) => {
      console.warn('[HydraBrain] user-memory extraction failed:', error);
    });
  }, delayMs);
}
...
function enqueueUserMemoryExtractionAfterTurn(payload) {
  queueMicrotask(() => {
    enqueueUserMemoryExtraction(payload).catch((error) => {
      console.warn('[HydraBrain] failed to enqueue user-memory extraction:', error);
    });
  });
}
...
async function loadCustomSkills() {
  ...
  if (retainedSkills.length !== storedSkills.length) {
    try {
      ...
      console.warn('[HydraBrain] Retired packaged skills could not be removed', error);
    }
  }
  ...
  try {
    ...
    console.warn('[HydraBrain] Default skills could not be loaded', e);
  }
  try {
    ...
    console.warn('[HydraBrain] Packaged skills could not be refreshed', e);
  }
...
async function showFirstInstallGuide(details) {
  ...
  await chrome.storage.local.set({ pinCoachmarkPending: true }).catch((error) => {
    console.warn('[HydraBrain] Could not prepare the first-open pin coachmark:', error);
  });
  try {
    ...
    console.warn('[HydraBrain] Could not open the first-install pinning guide:', error);
  }
}
...
chrome.runtime.onInstalled.addListener(async (details) => {
  ...
  console.log('[HydraBrain] Extension installed, providers loaded.');
});
...
chrome.storage.onChanged.addListener((changes) => {
  ...
  if (changes.providers || changes.activeProvider || changes.helpImproveHydraBrain) providerManager.load().catch(() => {});
  if (changes.hydrabrainCloudBridgeEnabled || changes.hydrabrainCloudBridgeUrl) {
  ...
  if (shouldClearUserMemoryExtractionQueueForChanges(changes)) {
    clearUserMemoryExtractionQueue().catch((error) => {
      console.warn('[HydraBrain] failed to clear user-memory extraction queue:', error);
    });
  }
  ...
  if (changes.capsolverApiKey || changes.captchaSolverEnabled) {
    loadCaptchaSolver()
      ...
      .catch((error) => console.warn('[HydraBrain] CapSolver setting could not be refreshed', error));
  }
...
//
// We tie the side panel to a per-window "HydraBrain" tab group rather than to
...
// `_addToHydraBrainGroup` behaviour for `new_tab` calls — so a sidebar
...
// Per-window HydraBrain group ID. windowId -> tabGroups groupId.
const hydraBrainGroupByWindow = new Map();
const HB_GROUPS_KEY = 'hydraBrainGroupByWindow';

async function loadHydraBrainGroups() {
  if (!chrome.tabGroups) return;
  try {
    const stored = await chrome.storage.session.get(HB_GROUPS_KEY);
...
//
// We do NOT have a "tab left the HydraBrain group → disable panel" path,
...
/**
 * Make sure `tab.windowId` has a HydraBrain group AND that `tab` is in it.
...
async function ensureHydraBrainGroup(tab) {
  ...
  try {
    ...
    if (groupId == null) {
      // Always create a FRESH HydraBrain group for this window, even if the
      ...
      // it to "HydraBrain" — surprising for users who had a "Dev" or
...
// the same panel visibility and HydraBrain group model.
chrome.runtime.onMessage.addListener((msg, sender) => {
  ...
  chrome.tabs.get(tabId).then((tab) => {
    ...
    savePanelTabs();
    ensureHydraBrainGroup(tab).catch(() => {});
  }).catch(() => {});
});
...
// "Stop HydraBrain" floating button. The chat / chat_stream / continue
...
function launchDetachedRun(action, msg, sender) {
  ...
  task.catch((error) => {
    ...
    console.warn(`[HydraBrain] detached ${action} run failed:`, error);
  }).finally(() => {
...
chrome.action.onClicked.addListener((tab) => {
  ...
  // gesture window for sidePanel.open, but ensureHydraBrainGroup doesn't
  // need it.
  ensureHydraBrainGroup(tab).catch(() => {});
});
...
// User ungrouped (or Chrome auto-collapsed) the HydraBrain group entirely.
...
globalThis.__hydrabrainLastNav = lastNavByTab;
...
globalThis.__hydrabrainApiRequests = apiRequestsByTab;
globalThis.__hydrabrainApiRequestReplay = apiRequestReplayById;
  ...
  switch (msg.action) {
      ...
      try {
        ...
        if (runCaptureState) {
          try {
            ...
            console.warn('[HydraBrain] trailing run capture failed to finish:', error);
    ...
    case 'export_config': {
      ...
      const config = createConfigExport(stored, {
        locale: msg.locale,
        hydrabrainVersion: chrome.runtime.getManifest().version,
      });
      return {
        ok: true,
    ...
    // ─── HydraBrain message handlers ──────────────────────────────────────────
    case 'hydrabrain_query': {
      try {
        const { HydraDBClient } = await import('./hydradb/client.js');
        const client = await HydraDBClient.fromStorage();
        const result = await client.query(msg.query || '', {
          collection: msg.collection,
          mode: msg.mode || 'fast',
          type: msg.type || 'knowledge',
          queryApps: true,
        });
    ...
    case 'hydrabrain_metrics': {
      try {
        const { HydraDBClient } = await import('./hydradb/client.js');
        const client = await HydraDBClient.fromStorage();
        return { ok: true, metrics: client.getMetrics() };
      } catch (err) {
        return { ok: false, error: err.message };
      }
    }

    case 'hydrabrain_sync': {
      try {
        const { HydraDBClient, HYDRADB_CONNECTORS } = await import('./hydradb/client.js');
        const client = await HydraDBClient.fromStorage();
        const connector = msg.connector || 'all';
        let results;
        if (connector === 'all') {
          results = await client.syncAllConnectors();
        } else {
          const connectorId = HYDRADB_CONNECTORS[connector];
          if (!connectorId) return { ok: false, error: `Unknown connector: ${connector}` };
          const r = await client.syncConnector(connectorId);
          results = { [connector]: r };
        }
        const synced = Object.entries(results).filter(([, r]) => !r.error).map(([n]) => n);
        const failed = Object.entries(results).filter(([, r]) => r.error).map(([n, r]) => `${n}: ${r.error}`);
    ...
    case 'hydrabrain_ingest_demo': {
      try {
        const { HydraDBClient } = await import('./hydradb/client.js');
        const { ingestAllDemoData } = await import('./hydradb/demo-data.js');
        const client = await HydraDBClient.fromStorage();
        const report = await ingestAllDemoData(client);
    ...
    case 'hydrabrain_demo': {
      try {
        const { HydraDBClient } = await import('./hydradb/client.js');
        const { DEMO_QUESTIONS, executeReasoningPlan } = await import('./hydradb/client.js');
        // eslint-disable-next-line no-shadow
        const { executeReasoningPlan: runPlan } = await import('./hydradb/client.js');
        const { DEMO_QUESTIONS: demoQs } = await import('./hydradb/demo-data.js');
        const client = await HydraDBClient.fromStorage();
        const results = [];
        for (const q of demoQs) {
          const trace = await runPlan(client, q.question, q.plan);
          results.push(trace);
        }
    ...
    case 'hydrabrain_page_sync': {
      // Request page content extraction from the active tab's content script
      const tabId = msg.tabId || sender.tab?.id;
      if (!tabId) return { ok: false, error: 'No tab ID' };
      try {
        const pageData = await chrome.tabs.sendMessage(tabId, { type: 'hydrabrain_sync_request' });
        if (!pageData?.success) return { ok: false, error: pageData?.error || 'Extraction failed' };
        const { HydraDBClient, HYDRADB_COLLECTIONS } = await import('./hydradb/client.js');
        const client = await HydraDBClient.fromStorage();
        const collection = HYDRADB_COLLECTIONS[pageData.source] || undefined;
        const result = await client.ingestText(pageData.text, {
          title:      pageData.title,
          collection,
          source:     pageData.source,
          metadata:   { url: pageData.url, source_type: pageData.source },
        });
        return { ok: true, ...result, source: pageData.source, title: pageData.title };
      } catch (err) {
        return { ok: false, error: err.message };
      }
    }
