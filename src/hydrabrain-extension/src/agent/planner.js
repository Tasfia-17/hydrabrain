...
export const PLANNER_SYSTEM_PROMPT = `You are the planning subsystem for HydraBrain, a browser automation agent. Given the user's task and current page context, output ONLY a single JSON object (no markdown fences, no commentary outside the JSON).
...
- allows_app_state_tool_evidence is true only when the requested work itself is reading/updating HydraBrain scratchpad or progress ledger (not incidental bookkeeping).
...
export const PLANNER_INTENT_SYSTEM_PROMPT = `You are the intent and compact planning subsystem for HydraBrain, a browser automation agent. Output ONLY one JSON object:
{
  "request_kind": "execute" | "respond" | "plan_only" | "clarify",
  "requires_state_change": boolean,
  "requires_submission": boolean,
