...
function isActiveRunError(error) {
  return /agent run is already in progress|active HydraBrain run/i.test(String(error?.message || error || ''));
}
...
export class ScheduledJobManager {
  ...
  start() {
    ...
    this.api?.alarms?.onAlarm?.addListener?.((alarm) => {
      ...
      run.catch((e) => {
        console.warn('[HydraBrain] scheduled job alarm failed:', e);
      });
      return run;
    });
    this.restoreAlarms().catch((e) => console.warn('[HydraBrain] restore scheduled alarms failed:', e));
  }
    ...
    if (keepWatching) {
      ...
      if (freshAlert) {
        try {
          ...
          console.warn('[HydraBrain] watch alert playback failed:', error);
        }
      }
      return;
    }
  ...
  async _runJob(jobId) {
    ...
    if (this._runningTabs.has(tabId) || this.agent.isRunning(tabId)) {
      await this._requeue(job, 'The target tab already has an active HydraBrain run.');
      return;
    }
    ...
    const onUpdate = (type, data) => {
      ...
      if (type === 'clarify') {
        ...
        this._updateJobIf(job.id, (prev) => prev.status === 'running', () => ({
          ...
          console.warn('[HydraBrain] failed to mark scheduled job as waiting for input:', e);
        });
        ...
        this._updateJobIf(job.id, (prev) => prev.status === 'needs_user_input', () => ({
          ...
          console.warn('[HydraBrain] failed to resume scheduled job after clarify timeout:', e);
        });
      }
    ...
    try {
      ...
      if (isActiveRunError(e)) {
        await this._requeue(running, 'The target tab already has an active HydraBrain run.');
      } else {
