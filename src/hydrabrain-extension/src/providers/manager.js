...
export class ProviderManager {
   ...
   */
  async load() {
    const data = await chrome.storage.local.get(['providers', 'activeProvider', HYDRABRAIN_DEVICE_GUID_KEY, HELP_IMPROVE_HYDRABRAIN_KEY]);
  ...
  async _getDeviceGuid(storedGuid) {
    ...
    try {
      await chrome.storage.local.set({ [HYDRABRAIN_DEVICE_GUID_KEY]: guid });
    } catch (e) {
