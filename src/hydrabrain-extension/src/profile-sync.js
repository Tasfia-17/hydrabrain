...
export const PROFILE_SYNC_KEYS = {
  enabled: 'profileSyncEnabled', token: 'profileSyncToken', deviceGuid: 'hydrabrainDeviceGuid',
...
const API = 'https://api.hydrabrain.one/v1/sync';
...
export class ProfileSyncManager {
  ...
  async authStatus(challengeId, verifier) { const q = new URLSearchParams({ challenge_id: challengeId }); const r = await this.request(`/auth/status?${q}`, { headers: { 'X-HydraBrain-Sync-Verifier': verifier } }); if (r.body.token) await this.storage.set({ [PROFILE_SYNC_KEYS.token]: r.body.token }); return r.body; }
