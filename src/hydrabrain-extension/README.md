...
- **Multi-Provider LLM** — HydraBrain Cloud plus local llama.cpp/Ollama/LM Studio/Jan/vLLM/SGLang/LocalAI and major direct cloud providers
...
```bash
git clone https://github.com/hydrabrain-one/hydrabrain.git
```

1. Open Chrome → `chrome://extensions/`
2. Enable **Developer mode** (top right)
3. Click **Load unpacked** → select the `hydrabrain/src/chrome` folder
...
3. Navigate to `hydrabrain/src/firefox/` and select `manifest.json`
...
# Or run: ollama launch hydrabrain --model <model>
...
Click the HydraBrain icon → the side panel opens. Type a message like:
...
- **Protected browser pages** — Browser-internal and extension-store pages restrict content-script/CDP access; HydraBrain reports these as protected instead of retrying DOM actions.
...
The repository `web/` folder contains the landing page for [hydrabrain.one](https://hydrabrain.one), deployable to Vercel:
