...
- optional tool schemas declared in fenced `hydrabrain-tools` JSON blocks.
...
recorder context through `getDisplayMedia()`, shows the HydraBrain recording banner
by default, and can be stopped by its Stop button or double Escape on HydraBrain or
...
or whole screen if they want the HydraBrain panel in the video.
...
background.js (on recorder-stop)
      ├─ chrome.downloads.download(dataURL → requested name or hydrabrain-recording-<ts>.webm)
...
CDP events are **trusted** (`event.isTrusted === true`). Many sites reject synthetic `el.click()`; CDP is what lets HydraBrain work on those.
...
`ProviderManager` seeds HydraBrain Cloud, seven local backends, Azure OpenAI, AWS
...
Off by default. Enabled via Settings → Display → "Record traces". When on, every agent run writes to an IndexedDB database (`hydrabrain-traces`):
