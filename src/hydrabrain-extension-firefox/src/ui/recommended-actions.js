...
function hydrabrainTweetRunOptions(postText) {
  const exactPost = String(postText || '').trim();
  return {
    id: 'tweet-hydrabrain',
    skipPlanner: true,
    tool: 'navigate',
    summary: 'Publish the reviewed localized HydraBrain post exactly as supplied.',
    steps: [
...
function hydrabrainLinkedInRunOptions(postText) {
  const exactPost = String(postText || '').trim();
  return {
    id: 'post-hydrabrain-linkedin',
    skipPlanner: true,
    tool: 'navigate',
    summary: 'Publish the reviewed localized HydraBrain post on LinkedIn exactly as supplied.',
    steps: [
...
export function buildRecommendedActions(pageInfo = {}, options = {}) {
  ...
  const actions = [];
  const hydrabrainPostText = t('sp.recommended.tweet.text');
  const hydrabrainPromotionVariant = options.hydrabrainPromotionVariant === 'linkedin' ? 'linkedin' : 'x';

  addUnique(actions, hydrabrainPromotionVariant === 'linkedin'
    ? {
      id: 'post-hydrabrain-linkedin',
      label: t('sp.recommended.linkedin.label'),
      prompt: t('sp.recommended.linkedin.prompt', { post: hydrabrainPostText }),
      mode: 'act',
      runOptions: hydrabrainLinkedInRunOptions(hydrabrainPostText),
    }
    : {
      id: 'tweet-hydrabrain',
      label: t('sp.recommended.tweet.label'),
      prompt: t('sp.recommended.tweet.prompt', { post: hydrabrainPostText }),
      mode: 'act',
      runOptions: hydrabrainTweetRunOptions(hydrabrainPostText),
    });
  ...
  if (!actions.some((action) => !['tweet-hydrabrain', 'post-hydrabrain-linkedin'].includes(action.id)) && pageInfo.title) {
