...
async function parseSlashCommands(text, tabId = currentTabId, options = {}) {
  ...
  if (command.value === '/export' && action === 'conversation') {
    ...
    const hydrabrainVersion = browser.runtime.getManifest().version || 'unknown';
...
async function sendToBackground(action, data = {}) {
  ...
  if (response == null) {
    throw new Error(`No response from HydraBrain background for "${action}". The background script may have restarted or crashed; reload the sidebar/extension and check the Firefox extension console for the original error.`);
  }
  if (response?.error) {
