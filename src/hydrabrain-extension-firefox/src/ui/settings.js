/**
 * HydraBrain Settings Page — provider configuration + display settings.
 */
...
const HYDRABRAIN_SUBSCRIBE_URL = 'https://hydrabrain.one/subscribe';
const HYDRABRAIN_ACCOUNT_URL = 'https://api.hydrabrain.one/account';
...
function hydrabrainSubscribeUrl(deviceGuid) {
  const url = new URL(HYDRABRAIN_SUBSCRIBE_URL);
  if (deviceGuid) {
...
function hydrabrainAccountUrl(deviceGuid) {
  const url = new URL(HYDRABRAIN_ACCOUNT_URL);
  if (deviceGuid) {
    url.searchParams.set('client_reference_id', deviceGuid);
  }
  return url.toString();
...
async function init() {
  // Migration: the old auth.hydrabrain.one sign-in stored a bearer token and
  ...
  if (helpImproveToggle) helpImproveToggle.checked = stored.helpImproveHydraBrain !== false; // on by default
...
helpImproveToggle?.addEventListener('change', async () => {
  await browser.storage.local.set({ helpImproveHydraBrain: helpImproveToggle.checked }).catch(() => {});
});
...
function describeProfileSyncState(state) {
  ...
  if (state.status === 'subscription') return 'HydraBrain Cloud membership is required for encrypted sync.';
  if (state.status === 'error') return state.error || 'Encrypted sync needs attention.';
  if (!state.authenticated) return 'Sign in with your HydraBrain Cloud email to use encrypted sync.';
...
async function requestProfileSyncDataConsent() { const permissions = await browser.permissions.getAll(); if (!Object.hasOwn(permissions, 'data_collection')) return window.confirm('Turn on encrypted sync? HydraBrain will transmit an end-to-end encrypted copy of your memories, profile autofill, and API-key provider settings to HydraBrain Cloud. Chat history and OAuth sign-ins are not synced.'); return browser.permissions.request({ data_collection: ['personallyIdentifyingInfo', 'authenticationInfo', 'personalCommunications', 'websiteContent', 'technicalAndInteraction'] }); }
...
if (btnExportUserMemory) {
  btnExportUserMemory.addEventListener('click', async () => {
    ...
    a.href = url;
    a.download = `hydrabrain-user-memory-${Date.now()}.json`;
...
function supportsProviderCompatibilitySettings(id, config = {}) {
  return id !== 'hydrabrain_cloud' && ['openai', 'llamacpp', 'azure_openai'].includes(config.type);
}
...
function renderProviders() {
  ...
  for (const [id, config] of entries) {
    ...
    const isConfigured = id !== 'hydrabrain_cloud' && config.configured === true;
    ...
    const subscribeHref = id === 'hydrabrain_cloud' ? hydrabrainSubscribeUrl(config.deviceGuid) : '';
    const accountHref = id === 'hydrabrain_cloud' ? hydrabrainAccountUrl(config.deviceGuid) : '';
    const billingButton = id === 'hydrabrain_cloud'
      ...
      : '';
    let providerNote = '';
    if (id === 'hydrabrain_cloud') {
      const linkStyle = 'color:var(--accent,#4A90D9);text-decoration:none;';
      const privacyLink = `<a href="https://hydrabrain.one/privacy" target="_blank" rel="noopener noreferrer"
              style="${linkStyle}">${escapeHtml(t('st.providers.hydrabrain_note.privacy_link'))}</a>`;
      const subscribeLink = `<a href="${escapeHtml(subscribeHref)}" target="_blank" rel="noopener noreferrer"
              style="${linkStyle}">hydrabrain.one/subscribe</a>`;
      const accountLink = `<a href="${escapeHtml(accountHref)}" target="_blank" rel="noopener noreferrer"
              style="${linkStyle}">api.hydrabrain.one/account</a>`;
      providerNote = `<div style="margin-top:10px;padding:10px 12px;border-radius:6px;
           ...
           ${t('st.providers.hydrabrain_data_use.body', { privacyLink, subscribeLink, accountLink })}
         </div>`;
    }
    ...
    const ollamaWarning = id === 'ollama'
      ? `<aside class="provider-warning provider-ollama-warning" role="note"
           ...
           <a href="https://www.hydrabrain.one/blog/ollama-launch-handoff"
...
async function saveProvider(id, { showFlash = true, markConfigured = true } = {}) {
  ...
  if (providersData[id]) {
    ...
    if (markConfigured) providersData[id].configured = id !== 'hydrabrain_cloud';
  }
...
function refreshProviderCardStatus(id) {
  ...
  if (!card) return;
  const isConfigured = id !== 'hydrabrain_cloud' && providersData[id]?.configured === true;
  const isSelected = id === activeProviderId;
