...
export const SAVED_WORKFLOW_SCHEMA = 'hydrabrain-workflow/1';
...
export function compileWorkflowFromTrace(run, events, options = {}) {
  ...
  const workflow = normalizeSavedWorkflow({
    ...
    source: {
      ...
      hydrabrainVersion: cleanText(run.hydrabrainVersion, 40),
    },
    start: urlScope,
    parameters,
    steps,
...
export function normalizeSavedWorkflow(input, options = {}) {
  ...
  return {
    ...
    source: {
      ...
      hydrabrainVersion: cleanText(input?.source?.hydrabrainVersion, 40),
    },
    start,
    parameters,
    steps,
