...
export function tracesToMarkdown(runsWithEvents, {
  title = 'HydraBrain Conversation — tool chain',
  notes = [],
  exportedByHydraBrainVersion = '',
} = {}) {
  const runs = Array.isArray(runsWithEvents) ? runsWithEvents : [];
  let md = `# ${title}\n\n`;
  const exportVersion = oneLine(exportedByHydraBrainVersion);
  if (exportVersion) md += `_Exported with HydraBrain v${exportVersion}_\n\n`;
  let turnCount = 0;
  let toolCount = 0;
  ...
  for (const entry of runs) {
    ...
    const recordedVersion = oneLine(run.hydrabrainVersion || '');
    const events = Array.isArray(entry.events) ? [...entry.events].sort((a, b) => (a?.seq || 0) - (b?.seq || 0)) : [];
    const meta = [
      recordedVersion ? `recorded with HydraBrain v${recordedVersion}` : 'recorded HydraBrain version unavailable',
      run.model,
