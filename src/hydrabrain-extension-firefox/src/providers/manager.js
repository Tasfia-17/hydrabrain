...
const HYDRABRAIN_CLOUD_PROVIDER_ID = 'hydrabrain_cloud';
const LOCAL_MODEL_LIST_PROVIDER_IDS = ['llamacpp', 'ollama', 'lmstudio', 'jan', 'vllm', 'sglang', 'localai', 'gpt4all'];
const HYDRABRAIN_CLOUD_CONTEXT_WINDOW = 1000000;
const HYDRABRAIN_CLOUD_LEGACY_CONTEXT_WINDOW = 256000;
const HYDRABRAIN_DEVICE_GUID_KEY = 'hydrabrainDeviceGuid';
const HELP_IMPROVE_HYDRABRAIN_KEY = 'helpImproveHydraBrain';
...
export class ProviderManager {
   ...
   */
  async load() {
    const data = await browser.storage.local.get(['providers', 'activeProvider', HYDRABRAIN_DEVICE_GUID_KEY, HELP_IMPROVE_HYDRABRAIN_KEY]);
    ...
    const legacyActiveProviderId = ['hydrabrain', 'openai_subscription'].includes(data.activeProvider)
      ? HYDRABRAIN_CLOUD_PROVIDER_ID
      : data.activeProvider;
    ...
    for (const [id, config] of Object.entries(defaults)) {
      ...
      const configured = id !== HYDRABRAIN_CLOUD_PROVIDER_ID && (
    ...
    for (const [id, config] of Object.entries(stored)) {
      if (!configs[id] && this._isSupportedProviderConfig(id, config)) {
        ...
        configs[id] = {
          ...config,
          configured: id !== HYDRABRAIN_CLOUD_PROVIDER_ID && (config.configured === true || !hasConfiguredMarker),
        };
        if (!hasConfiguredMarker) providerStateMigrated = true;
      }
    }
    delete configs.hydrabrain;
    ...
    if (configs[HYDRABRAIN_CLOUD_PROVIDER_ID]) {
      configs[HYDRABRAIN_CLOUD_PROVIDER_ID].deviceGuid = await this._getDeviceGuid(data[HYDRABRAIN_DEVICE_GUID_KEY]);
      configs[HYDRABRAIN_CLOUD_PROVIDER_ID].helpImproveHydraBrain = data[HELP_IMPROVE_HYDRABRAIN_KEY] !== false;
    }
    this.activeProviderId = legacyActiveProviderId || HYDRABRAIN_CLOUD_PROVIDER_ID;
    if (!configs[this.activeProviderId]) this.activeProviderId = HYDRABRAIN_CLOUD_PROVIDER_ID;
    if (this.activeProviderId !== HYDRABRAIN_CLOUD_PROVIDER_ID && configs[this.activeProviderId]?.configured !== true) {
      this.activeProviderId = HYDRABRAIN_CLOUD_PROVIDER_ID;
  ...
  _defaultConfigs() {
    return {
      hydrabrain_cloud: {
        type: 'openai',
        category: 'cloud',
        label: 'HydraBrain Cloud',
        providerName: 'hydrabrain-cloud',
        baseUrl: 'https://api.hydrabrain.one/v1',
        model: 'hydrabrain-cloud 1.0',
        contextWindow: HYDRABRAIN_CLOUD_CONTEXT_WINDOW,
        inputCostPerMillionUsd: 0.20,
        outputCostPerMillionUsd: 1.15,
        supportsStreamUsageOptions: true,
        supportsAskStreaming: true,
        supportsVision: true,
        // HydraBrain Cloud proxies to OpenRouter, whose upstream models
  ...
  _migrateStoredProviderConfigs(stored) {
    ...
    // Existing installs stored omitToolsWhenImagesPresent:true for HydraBrain
    ...
    if (migrated.hydrabrain_cloud?.omitToolsWhenImagesPresent) {
      migrated.hydrabrain_cloud = {
        ...migrated.hydrabrain_cloud,
        omitToolsWhenImagesPresent: false,
      };
    }
    if (Number(migrated.hydrabrain_cloud?.contextWindow) === HYDRABRAIN_CLOUD_LEGACY_CONTEXT_WINDOW) {
      migrated.hydrabrain_cloud = {
        ...migrated.hydrabrain_cloud,
        contextWindow: HYDRABRAIN_CLOUD_CONTEXT_WINDOW,
      };
  ...
  async updateProvider(id, config, { markConfigured = true } = {}) {
    ...
    const merged = {
      ...
      configured: id !== HYDRABRAIN_CLOUD_PROVIDER_ID && (markConfigured || current.configured === true),
    };
  ...
  async _getDeviceGuid(storedGuid) {
    ...
    try {
      await browser.storage.local.set({ [HYDRABRAIN_DEVICE_GUID_KEY]: guid });
    } catch (e) {
  ...
  async _guidFromMaterial(material) {
    const bytes = new TextEncoder().encode(`hydrabrain-device-v1:${material}`);
