...
/**
 * HydraBrain Background Script (Firefox)
...
function createContextMenus() {
    ...
    try {
      ...
      Promise.resolve(result).catch((e) => {
        if (!/duplicate/i.test(String(e?.message || e))) {
          console.warn('[HydraBrain] Failed to create context menu:', e?.message || e);
        }
      });
    } catch (e) {
      if (!/duplicate/i.test(String(e?.message || e))) {
  ...
  const create = () => {
    ...
    createItem({ id: 'hydrabrain-selection-separator-1', parentId: CONTEXT_MENU_ASK_SELECTION_ID, type: 'separator', contexts: ['selection'] });
    ...
    createItem({ id: 'hydrabrain-selection-separator-2', parentId: CONTEXT_MENU_ASK_SELECTION_ID, type: 'separator', contexts: ['selection'] });
...
// ────────────────────────────────────────────────────────────────────────
// Tab grouping (visual scope for a HydraBrain session)
//
...
// seeds) a colored "HydraBrain" tab group for that window. Agent-spawned
// tabs (new_tab tool, target=_blank redirects) auto-join the same group
// via agent.js's `_addToHydraBrainGroup`. The group label is what tells
// the user at a glance "this is part of a HydraBrain session".
//
...
/**
 * Make sure `tab.windowId` has a "HydraBrain" group AND that `tab` is in
...
// HydraBrain grouping as browser-action opens.
...
function openSidebarForContextMenu(tab) {
  ...
  if (tab?.id) ensureHydraBrainGroup(tab).catch(() => {});
}
...
// user opens HydraBrain manually.
...
// in the HydraBrain group so the colored label appears immediately.
...
// HydraBrain" floating button. The chat / chat_stream / continue handlers
