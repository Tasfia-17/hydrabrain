/**
 * HydraBrain Content Script
 * Injected into every page — handles page reading and DOM actions.
 */

(() => {
  // Prevent double-injection
  if (window.__hydrabrain_injected) return;
  window.__hydrabrain_injected = true;
  ...
  function showAgentWorkingTarget(el, source = 'interaction') {
    try {
      window.__hydrabrainAgentIndicator?.showTarget?.(el, source);
    } catch {}
  }
   ...
   * file directly (or presents HydraBrain's own picker when no downloadId is
  ...
  function clickWithoutNativeFilePicker(runClick, settleMs = FILE_PICKER_GUARD_SETTLE_MS) {
    ...
    const installPageShowPickerGuard = () => {
      ...
      if (!root) return () => {};
      const guardAttr = 'data-hydrabrain-file-picker-guard';
      const blockedAttr = 'data-hydrabrain-file-picker-blocked';
      const blockedEvent = 'hydrabrain:file-picker-guard-blocked';
      const armPageGuard = () => {
        root.setAttribute(guardAttr, guardId);
        document.dispatchEvent(new Event('hydrabrain:file-picker-guard-arm'));
      };
      const onBlocked = () => {
        try {
      ...
      return (disarmPageGuard = true) => {
        if (disarmPageGuard) {
          document.dispatchEvent(new Event('hydrabrain:file-picker-guard-disarm'));
        }
  ...
  function filePickerBlockedResponse(blocked, label = '') {
    ...
    const guidance = selector
      ? `Call upload_file({selector: ${JSON.stringify(selector)}, downloadId: N}) directly; it attaches the downloaded file without opening an OS dialog. If there is no downloadId, call upload_file({selector: ${JSON.stringify(selector)}}) to use HydraBrain's own picker.`
    ...
    try {
      ...
      return {
        ...
        warning: verified
          ...
          : 'window.find reported a match, but HydraBrain could not verify a visible current selection in the top document (for example, the active match may be inside a frame while an older top-document selection remains). Do not claim it is visibly highlighted. The browser Find UI was not opened.',
      };
    } catch (error) {
  ...
  function _waitForRichTextToolbarFocusedChildFrame(params = {}) {
    ...
    return new Promise(resolve => {
      ...
      const onMessage = event => {
        if (event?.data?.__hydrabrainFocusedFrameToken !== token) return;
  ...
  function _announceRichTextToolbarFocusedChildFrame(params = {}) {
    ...
    try {
      window.parent.postMessage({ __hydrabrainFocusedFrameToken: token }, '*');
